...
( function( $, api ) {
	...
	var $style     = $( '#ysPoet-colors-css' ),
	    $rgbaStyle = $( '#ysPoet-colors-css-rgba' ),
	    $body      = $( 'body' );

	if ( ! $style.length ) {

		$style = $( 'head' ).append( '<style type="text/css" id="ysPoet-colors-css" />' ).find( '#ysPoet-colors-css' );

	}

	if ( ! $rgbaStyle.length ) {

		$rgbaStyle = $( 'head' ).append( '<style type="text/css" id="ysPoet-colors-css-rgba" />' ).find( '#ysPoet-colors-css-rgba' );
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'ysPoet-update-colors-css', function( css ) {

			$style.html( css );

		} );

		api.preview.bind( 'ysPoet-update-colors-css-rgba', function( rgbaCSS ) {
